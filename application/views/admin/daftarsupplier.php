...
<body class="hold-transition skin-black sidebar-mini">
  ...
  <!-- ketiga -->
  <div class="modal fade" id="modal_edit_supplier" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
          <h4 class="modal-title" id="modal_edit_supplier"></h4>
        </div>
        <form class="" action="<?php echo base_url('admin/update_supplier')?>" method="post">
          <div class="modal-body">
            <input type="hidden" id="id_supplier" name="id_supplier" value="">
            <div class="form-group">
              <label for="">Nama Supplier</label>
              <input type="text" class="form-control" id="nama_supplier" placeholder="" name="nama_supplier">
            </div>
            <div class="form-group">
              <label for="">Alamat Supplier</label>
              <input type="text" class="form-control" id="alamat_supplier" placeholder="" name="alamat_supplier">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <input type="text" class="form-control" id="nama_instansi" placeholder="" name="nama_instansi">
            </div>
          </div>
              ...
              <h1 class="box-title">Daftar Supplier</h1>
                <div class="box-tools">
                  <div class="input-group input-group-sm" style="width: 150px;">
                      <a href="<?php echo base_url('inputsupplier')?>" class="btn btn-primary">
              ...
              <table class="table table-hover">
                  ...
                  <tbody>
                    <?php foreach ($supplier as $inputsupplier):?>
                      <tr>
                        <td class="text-left"><?php echo $inputsupplier->nama_supplier?></td>
                        <td class="text-left"><?php echo $inputsupplier->alamat_supplier?></td>
                        <td class="text-left"><?php echo $inputsupplier->no_telp?></td>
                        <td class="text-left"><?php echo $inputsupplier->nama_instansi?></td>
                        <td class="text-center">
                          <!-- delete ketiga (cuma ubah yg action doang lho, tapi pastiin ada input type hidden sama submit) -->
                          <form class="" action="<?php echo base_url('admin/delete_supplier')?>" method="post">
                            <!-- kelima -->
                            <button type="button" class="btn btn-warning" onclick="edit_supplier('<?php echo $inputsupplier->id_supplier;?>')">
                              <i class="fa fa-edit"></i> Edit
                            </button>
                            <input type="hidden" name="id_supplier" value="<?php echo $inputsupplier->id_supplier; ?>">
                            <button type="submit" class="btn btn-danger" name="hapus_supplier">
...
$('table').DataTable({
  ...
  function edit_supplier(id_supplier) {
    $.ajax({
      type: 'get',
      url : '<?php echo base_url('admin/edit_supplier?id_supplier=')?>'+id_supplier,
      success: function(data) {
        $response = $(data);
        // ambil data dari url admin/edit_supplier?id_supllier=(sekian)
        var id_supplier = $response.filter('#id_supplier').text();
        var nama_supplier = $response.filter('#nama_supplier').text();
        var alamat_supplier = $response.filter('#alamat_supplier').text();
        var no_telp = $response.filter('#no_telp').text();
        var nama_instansi = $response.filter('#nama_instansi').text();
